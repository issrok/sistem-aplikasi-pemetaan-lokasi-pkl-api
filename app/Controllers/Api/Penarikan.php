...
class Penarikan extends BaseController
{
    ...
    public function upload_gambar($id)
    {
        ...
        if ($fileImage->isValid() && !$fileImage->hasMoved()) {
            ...
            $data['pendaftaran_penarikan_photo'] = $newName;
        }

        $get_photo_exist = $this->model->where('pendaftaran_id', $id)->get()->getRowArray();
        if (!(empty($get_photo_exist['pendaftaran_penarikan_photo'])) && file_exists(ROOTPATH . self::path_image_pendaftaran . $get_photo_exist['pendaftaran_penarikan_photo'])) {
            unlink(ROOTPATH . self::path_image_pendaftaran . $get_photo_exist['pendaftaran_penarikan_photo']);
        }
